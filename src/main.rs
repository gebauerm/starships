...
// Makes a random Vector2 with a given max magnitude
...
/// Break in pixels per second
const PLAYER_BREAK: f32 = 90.0;
...
fn player_handle_input(actor: &mut Actor, input: &InputState, dt: f32) {
    ...
    // This is used to make sure that the game runs at the same speed
    ...
    if input.yaxis > 0.0 {
        ...
        player_break(actor, dt)
    }
}

fn player_break(actor: &mut Actor, dt: f32) {
    let direction_vector = actor.velocity.normalize_or_zero();
    let break_vector = direction_vector * (PLAYER_BREAK);
    actor.velocity -= break_vector * dt;
    if actor.velocity.length_squared() < 40.0 {
...
const MAX_PHYSISCS_VEL: f32 = 250.0;

fn update_actor_position(actor: &mut Actor, dt: f32) {
    // the vector length is a norm and would therefore require sqrt
    // since we want to avoid sqrt out of performance concerns, we compare the squared length to the squared max velocity
    let norm_sq = actor.velocity.length_squared();
    if norm_sq > MAX_PHYSISCS_VEL.powi(2) {
        // this ensures maximum speed if above maximum speed
        actor.velocity = actor.velocity / norm_sq.sqrt() * MAX_PHYSISCS_VEL;
    }
    // dt = delta time, the time elapsed in the iteration
    let dv = actor.velocity * dt;
    // adding vectors stacks them upon each other, so this adds the change in position to the current position
    actor.pos += dv;
    actor.facing += actor.angel_vel;
}

/// Takes an actor and wraps its position to the bounds of the
/// screen, so if it goes off the left side of the screen it
/// will re-enter on the right side and so on.
/// achieved by checking hte position and then substracting or adding the vector.
/// Movement direction is determined by the sign of the position,
/// so if the position is positive we substract the screen size and if it is negative we add the screen size.
/// This ensures that the actor will re-enter on the opposite side of the screen.
...
fn world_to_screen_coords(screen_width: f32, screen_height: f32, point: Point2) -> Point2 {
    ...
    let y: f32 = screen_height - (point.y + screen_height / 2.0);
    Point2::new(x, y)
}
...
impl MainState {
    fn new(ctx: &mut Context) -> GameResult<MainState> {
        println!("Game resoure path: {:?}", ctx.fs);

        print_instructions();

        // Seed our RNG (similar to Python's random.seed(os.urandom(8)))
        // 1. seed = [0] * 8           -> create empty byte array
        // 2. getrandom.fill(seed)     -> os.urandom(8) fills it with random bytes
        // 3. Rand32.new(seed)         -> random.Random(seed) creates RNG instance
        let mut seed: [u8; 8] = [0; 8];
        getrandom::fill(&mut seed[..]).expect("Could not generate RNG seed");
...
impl EventHandler for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        while ctx.time.check_update_time(DESIRED_FPS) {
            ...
            // Update tje player state based on user input
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        let velo_dest = Point2::new(300.0, 10.0);

        let level_str = format!("Level: {}", self.level);
        let score_str = format!("Score: {}", self.score);
        let velo_str = format!("Velo: {}", self.player.velocity);
        ...
        canvas.draw(
            &graphics::Text::new(velo_str),
            graphics::DrawParam::from(velo_dest),
        );
    ...
    fn key_down_event(
        ...
        match input.event.logical_key {
            ...
            Key::Named(NamedKey::ArrowDown) => self.input.yaxis = -1.0,
...
fn main() -> GameResult {
    ...
    let cb = ContextBuilder::new("astroblast", "ggez")
        .window_setup(conf::WindowSetup::default().title("Astroblast!"))
